...
export const get_medication_by_name = async (req, res, next) => {
    try {
        const { medication_name } = req.params;

        const results = await Medication.findOne({ name: medication_name }).lean();
        res.json({ success: true, data: !!results });
    } catch (error) {
        next(error);
    }
